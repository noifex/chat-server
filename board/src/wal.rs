...
pub enum Recover {
    Clean,
    TruncatedTail { dropped_bytes: u64 },
    Poison(Poison),
}
...
impl Locked<'_> {
    ...
    pub fn recover(&mut self) -> Recover {
        ...
        loop {
            ...
            if content.is_empty() && has_nl {
                ...
                return Recover::Poison(Poison {
                    reason: format!("corrupt event a byte {last_good}"),
                });
            } else {
    ...
    pub fn load(&self) -> Result<(BTreeMap<u64, Task>, u64), Poison> {
        ...
        for line in reader.lines() {
            ...
            if ev.seq != expected {
                return Err(Poison {
                    reason: format!("seq gap: expected {expected}, got {}", ev.seq),
                });
            }
...
mod tests {
    ...
    use super::*;
    use crate::events::{Event,State, Kind};
    ...
    fn good_line(seq: u64, kind: Kind, task_id: u64) -> String {
        ...
        let mut s = serde_json::to_string(&ev.unwrap()).unwrap();
        s.push('\n');
        s // this is ok 
    }
    ...
    const SCHEMA_V1_HAPPY_PATH: &str = include_str!("../tests/fixtures/schema-v1-happy-path.jsonl");

    #[test]
    fn replays_schema_v1_fixture() {
        let path = tmp_wal();
        std::fs::write(&path, SCHEMA_V1_HAPPY_PATH).unwrap();
        let _tmp = TempWal { path: path.clone() };
        let mut wal = Wal::open(path);
        let recovered = wal.with_exclusive(|w| w.recover()).unwrap();
        match recovered {
            Recover::Clean=>{}
            Recover::TruncatedTail { dropped_bytes }=>{
                panic!(
                    "schema v1 fixture must end with a newline; dropped {dropped_bytes}
                    bytes"
                );
            }
            Recover::Poison(p)=>{
                panic!("schema v1 fixture is invalid: {}", p.reason);
            }
        }

        let (tasks, max_seq) = wal.with_exclusive(|w| w.load()).unwrap().unwrap();
        assert_eq!(max_seq, 5);
        let task = tasks.get(&1).expect("fixture contains task 1");
        assert_eq!(task.state, State::Done);
        assert_eq!(task.desc, "schema v1 fixture task");
        assert_eq!(task.owner.as_deref(), Some("Coffee"));
        assert_eq!(task.author.as_deref(), Some("Coffee"));
        assert_eq!(task.commit_sha.as_deref(), Some("0123456789abcdef"));
    }
    #[test]
    fn torn_tail_recovers() {
        let torn = serde_json::to_string(
            &Event::new(3, Kind::TaskAdded, 3, "tester".to_string()).unwrap(),
        )
        .unwrap();
        let path = write_wal(&[
