...
use crate::events::{Event, Kind, State, Task};
    ...
    match ev.kind {
        ...
        Kind::Approve => {
            ...
            match &task.author {
                ...
                _ => Err(bad(format!(
                    "task {} cannot be approved by author or unowned",
                    ev.task_id
                ))),
            }
        }
        ...
        Kind::Compensate => {
            ...
            match &task.author {
                ...
                _ => Err(bad(format!(
                    "task {} cannot be compensated by author or unowned",
                    ev.task_id
                ))),
            }
        }
        ...
        Kind::ChangesRequested => {
            ...
            if task.state != State::Review {
                return Err(bad(format!(
                    "task {} is {:?}, cannot request changes",
                    ev.task_id, task.state
                )));
            }
            match &task.author {
                ...
                _ => Err(bad(format!(
                    "task {} cannot be sent back by author or unowned",
                    ev.task_id
                ))),
            }
        }
...
fn get(map: &mut BTreeMap<u64, Task>, id: u64) -> Result<&mut Task, Poison> {
    map.get_mut(&id)
        .ok_or_else(|| bad(format!("task {id} not found")))
}
...
mod tests {
    ...
    fn done_task() -> BTreeMap<u64, Task> {
        ...
        apply(
            &mut map,
            &Event::new(1, Kind::TaskAdded, 1, "sys".into()).unwrap(),
        )
        .unwrap();
        apply(
            &mut map,
            &Event::new(2, Kind::Claimed, 1, "Coffee".into()).unwrap(),
        )
        .unwrap();
        apply(
            &mut map,
            &Event::new(3, Kind::Working, 1, "Coffee".into())
                .unwrap()
                .with_fencing(Some(2)),
        )
        .unwrap();
        apply(
            &mut map,
            &Event::new(4, Kind::Review, 1, "Coffee".into())
                .unwrap()
                .with_fencing(Some(2)),
        )
        .unwrap();
        apply(
            &mut map,
            &Event::new(5, Kind::Approve, 1, "Cola".into())
                .unwrap()
                .with_commit_sha("deadbeef".into()),
        )
        .unwrap();
        map
    }
    ...
    fn saga_done_to_rolled_back() {
        ...
        apply(
            &mut map,
            &Event::new(6, Kind::Compensate, 1, "Tea".into()).unwrap(),
        )
        .unwrap();
        assert_eq!(map[&1].state, State::Compensating);
        apply(
            &mut map,
            &Event::new(7, Kind::RolledBack, 1, "Coffee".into())
                .unwrap()
                .with_fencing(Some(6)),
        )
        .unwrap();
    ...
    fn saga_owner_cannot_compensate() {
        ...
        let r = apply(
            &mut map,
            &Event::new(6, Kind::Compensate, 1, "Coffee".into()).unwrap(),
        );
        assert!(r.is_err());
        assert_eq!(map[&1].state, State::Done);
    }
    ...
    fn reclaim_cannot_launder_self_approval() {
        ...
        apply(
            &mut map,
            &Event::new(5, Kind::Reclaimed, 1, "Tea".into()).unwrap(),
        )
        .unwrap();
        assert_eq!(map[&1].state, State::Review);
        ...
        let r = apply(
            &mut map,
            &Event::new(6, Kind::Approve, 1, "Coffee".into()).unwrap(),
        );
        assert!(r.is_err());
        assert_eq!(map[&1].state, State::Review);
    ...
    fn reclaim_before_review_lets_writer_approve() {
        ...
        apply(
            &mut map,
            &Event::new(4, Kind::Reclaimed, 1, "Tea".into()).unwrap(),
        )
        .unwrap();
        assert_eq!(map[&1].author, None);
        apply(
            &mut map,
            &Event::new(5, Kind::Review, 1, "Tea".into())
                .unwrap()
                .with_fencing(Some(4)),
        )
        .unwrap();
